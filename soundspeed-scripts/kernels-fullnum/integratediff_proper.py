...
outfile='/home/fverdian/class/soundspeed-scripts/kernels-fullnum/proper-chi-oneloop-results/kJ'+str(kref).replace('.','p')+'-fx'+str(fx).replace('.','p')+'.txt'
if os.path.exists(outfile):
    os.remove(outfile)
open(outfile, 'a').write(f'# k, EdS result, numerical-EdS difference, PLpurecdm, PL\n')
...
for kEval in kevList:
    ...
    open(outfile, 'a').write(f'{kEval} {train.mean} {result.mean} {Pcdmonly_int(kEval)} {PLc_int(kEval)}\n')
