...
if not args.nosave:
    ...
    if os.path.exists(outfile):
        os.remove(outfile)
    open(outfile, 'a').write(f'# k, dcdc22, dcdc22_EdS, dcTc22, dcTc22_EdS, dcdx22, dcdx22_EdS, dcTx22, dcTx22_EdS, PL_cc, TL_x \n')
...
def training_int(vars,kev):
    ...
    P22dcdc=0; P22dcTc=0; P22dcdx=0; P22dcTx=0
    if kMq>q:
    ...
    intf=4*q*np.pi/fact
    return [intf*P22dcdc,intf*P22dcTc,intf*P22dcdx,intf*P22dcTx]
...
for kEval in kevList:
    ...
    traindcdc22,traindcTc22,traindcdx22,traindcTx22 = integ(functools.partial(training_int, kev=kEval),neval=10000)
    start_time = time.time()
    result = integ(functools.partial(Pnum_int, kev=kEval), neval=args.neval)
    resdcdc22, resdcTc22, resdcdx22, resdcTx22 = result

    print(f'At k={kEval:.3f} done (took {int((time.time()-start_time)//60)}m {(time.time()-start_time)%60:.0f}s)', flush=True)

    if not args.nosave: open(outfile, 'a').write(f'{kEval:.5g} {resdcdc22.mean:.5g} {traindcdc22.mean:.5g} {resdcTc22.mean:.5g} {traindcTc22.mean:.5g} {resdcdx22.mean:.5g} {traindcdx22.mean:.5g} {resdcTx22.mean:.5g} {traindcTx22.mean:.5g} {PLc_int(kEval):.5g} {nk.g_an(-2*np.log(kEval/kref)):.5g}\n')
