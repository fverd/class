...
kevList=np.logspace(np.log10(0.1),np.log10(0.4),30)
...
if not args.nosave:
    outfile='/home/fverdian/class/soundspeed-scripts/numerical-integrals/cutofftests/P13_varykmax-ma'+str(-np.log10(ma)).replace('.','p')+'-fx'+str(fx).replace('.','p')+'-N'+str(args.neval)+'-p'+str(p)+'.txt'
...
for kEval in kevList:
    integ = vegas.Integrator([[np.log(0.4), np.log(1.5)], [0., 1.]],mpi=True, nproc=70)
