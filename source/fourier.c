...
int fourier_init(
 ...
 class_alloc(pvecback_lya,pba->bg_size*sizeof(double),pfo->error_message);

  class_call(background_at_tau(pba,tau_lya,long_info,inter_normal,&last_index,pvecback_lya),
             pba->error_message,
             pfo->error_message);


  Hubble = pvecback_lya[pba->index_bg_H];
  pfo->kp_lya = pfo->kp_km_per_s*_c_/1000/(1.+pfo->zp_lya)*Hubble; //in Mpc^-1
  kp_lya_cubed=pfo->kp_lya*pfo->kp_lya*pfo->kp_lya;
  free(pvecback_lya);

  class_call(fourier_pk_tilt_at_k_and_z(pba,
                                          ppm,
                                          pfo,
                                          pk_linear,
                                          pfo->kp_lya,
  ...
  class_call(fourier_pk_at_k_and_z(pba,
             ...
             pfo->error_message,
             pfo->error_message);
  pfo->Delta_Lsquared_lya *= kp_lya_cubed/2/_PI_/_PI_;
  }
  ...
  if (pfo->fourier_verbose>0) {
    ...
    if (pfo->has_pk_m == _TRUE_)
      fprintf(stdout," -> sigma8=%g for total matter (computed till k = %g h/Mpc)\n -> S8=%g for total matter (computed till k = %g h/Mpc) with Omega_m = %g \n",
              pfo->sigma8[pfo->index_pk_m],  pfo->k[pfo->k_size-1]/pba->h,
              pfo->sigma8[pfo->index_pk_m]*pow((pba->Omega0_m/0.3),0.5),
